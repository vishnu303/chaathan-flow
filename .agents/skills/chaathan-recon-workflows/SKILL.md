...
- **Step 13: `js_deep_analysis`** (jsluice AST-based JS URL/object extraction + strict-format secret scanning with live validation + source map harvesting + subdomain extraction; priority-ranked top-5000 JS URLs; configurable via `general.js_analysis` in config.yaml). JS URLs are gathered from crawler outputs **and ffuf discoveries**, streamed line-by-line so huge wayback/gau outputs never load fully into memory. Secret validation is provider-correct: AWS keys are verified with a SigV4-signed STS call (a bare 403 never confirms), JWTs are never marked confirmed, and Slack webhooks are probed non-destructively. Provider-validatable patterns (aws-keys, github, stripe, google-api, slack-webhook, firebase, jwt) are ranked ahead of generic ones before the validation limit is applied, and the same false-positive context gate (Datadog RUM tokens etc.) runs on both JS bodies and source-map contents. Source-map candidates honor both modern `//# sourceMappingURL=` and legacy `//@ sourceMappingURL=` directives plus the `.map` fallback. Firebase secrets match both legacy `*.firebaseio.com` and current `*.firebasedatabase.app` RTDB domains. Late-discovered subdomains are scope-filtered and synced to the subdomains table with source `js`; appends to `all_subdomains.txt` skip entries already present so the product file never accumulates duplicates.
- **Step 14: `dir_fuzzing`** (ffuf directory fuzzing on up to 1000 live hosts; auto-detects SecLists on device if `--wordlist` is omitted). Fuzzing results write to `ffuf_discovered_urls.txt`. The total fuzzing budget is split into fair per-host time budgets (`total / host count`, floor 2 minutes) so early hosts cannot starve the rest; when the floor pushes host count × floor past the total budget, the ROI-ranked list is truncated to the hosts that fit and the remainder is logged as skipped.
...
- **Secret handling in scan state**: the persisted run-config JSON stores auth options as presence booleans only (`custom_cookie`/`custom_token` are `true`/`false`, headers as a boolean) — never values. Resuming a scan that used auth logs a warning telling the user to pass the flags again; the GitHub token is likewise resolved from config.yaml at resume time, never from state.
